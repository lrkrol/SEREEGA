...
%
%       Gate function to plot class activation patterns. Takes a class
...
function class = plot_signal(class, epochs)

% seeing if type can be determined
if ~isfield(class, 'type')
    error('class type not set');
end

% calling type-specific plot function
if ~exist(sprintf('%s_plot_signal', class.type))
    error('no plotting function found for class type ''%s''', class.type);
else
    plot_signal = str2func(sprintf('%s_plot_signal', class.type));
    h = plot_signal(class, epochs);
end
