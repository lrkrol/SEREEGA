...
labels = {};
fid = fopen('chanlocs-nyhead231.elp','r'); 
while ~feof(fid)
    line = fgets(fid);
...
[~, ~, ~, structout] = inputgui( ...
        'geometry', { 1 1 1 1 1 1 [4 1] }, ...
        'geomvert', [1 1 1 1 6 1 1], ...
        'uilist', { ...
                { 'style', 'text', 'string', 'Obtain a lead field from the New York Head', 'fontweight', 'bold' }, ...
                { }, ...
...
if ~isempty(structout)
    ...
    EEG.etc.sereega.leadfield = lf_generate_fromnyhead('labels', labelselection);
    
    % also adding code to generate lead field
    EEG.etc.sereega.leadfieldfunction = sprintf('lf_generate_fromnyhead(''labels'', %s);', ['{''' joinstring(labelselection, ''',''') '''}']);
    disp('Done.');
end
